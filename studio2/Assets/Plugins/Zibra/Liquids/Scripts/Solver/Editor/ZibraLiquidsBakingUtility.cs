...
{
    public class ZibraLiquidsBakingUtility : EditorWindow
    {
        ...
        [MenuItem("Zibra AI/Zibra AI - Liquids/Baking Utility (experimental)", false, 10)]
        public static void ShowWindow()
        {
            GetWindow<ZibraLiquidsBakingUtility>("Zibra Liquids Baking Utility (experimental)");
        }

        private void UpdateSelectedObject()
        {
            // If we only have single liquid instance, just set it right away save few clicks
            ZibraLiquid[] liquids = FindObjectsOfType<ZibraLiquid>();
            if (liquids.Length == 1)
            {
        ...
        private byte[] ConvertInitialStateToBytes(ZibraLiquid.BakedInitialState initialStateData)
        {
            ...
            // int - particleCount
            // int - header
            int byteCount = particleCount * (sizeof(float) * 3 + sizeof(int) * 2) + 2 * sizeof(int);

            byte[] output = new byte[byteCount];

            int header = ZibraLiquid.BAKED_LIQUID_HEADER_VALUE;
        ...
        void SaveState(bool saveToNewFile)
        {
        ...
        IEnumerator RunSimulation()
        {
            ...
            {
                if (EditorUtility.DisplayCancelableProgressBar("Zibra Liquids Simulation", "Simulation in progress...",
        ...
        void OnGUI()
        {
            ...
            if (liquidInstance != null)
            {
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Selected instance is not enabled, enable liquid instance and try again.",
                        MessageType.Error);
                    return;
                }
